...
def main():
    ...
    ap.add_argument("--agent-permission-mode",
                    ...
                    help="agent 权限模式（默认 default）：SDK 流式下 claude 无终端，未预允许的"
                         "工具直接自动拒绝（实测写文件被拒、agent 正常汇报，安全兜底）——agent.py "
                         "已默认放行 PowerShell/Bash/Read/Write/Edit/Glob/Grep/WebFetch/WebSearch/"
                         "Skill 供技能使用（Windows 双 shell 都能跑，模型可能走 Bash 或 PowerShell），"
                         "要放行其他具体工具在 agent.py 的 allowed_tools "
                         "白名单列它（不触发硬门），或 acceptEdits（仅自动接受文件编辑）/ "
    ...
    # ---- live2d 桌宠联动（须先于 tts：全 TTS 文本经 SayTTS 代理送到说话框）----
    ...
    if live2d is not None and live2d.enabled:
        ...
        idle_tts = SayTTS(tts, live2d.say, idle_cb=_round_idle)
        tts = idle_tts
