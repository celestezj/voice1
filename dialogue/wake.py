...
- 静默超时判定：对话期 `feed_decision` 里，系统未在应答（not engaged：LLM 未在思考且
  TTS 未在播）且 `now - last_activity > inactive_timeout` → 回休眠；"用户语音"信号 =
  ASR 出字（`note_partial`，任意距离都算）或 mic 块能量超阈值。
- **系统应答期（engaged = LLM 读流在途或 TTS 播放）刷新静默计时、不判超时**：否则慢查询
  （LLM 想很久才出音频，如 agent 跑天气脚本）期间用户不说话会被误判"静默超时"回休眠，
  答完立刻播"退下"告别语——"刚答完就退下"根因（2026-09-09 实测）。调用方传
  `ctrl.turn_active` 作 engaged。
"""
...
class WakeSession:
    ...
    def feed_decision(self, now, rms2, speech_pow, engaged):
        ...
        engaged = 系统正在应答（LLM 读流在途或 TTS 播放，调用方传 `ctrl.turn_active`）。
        应答期刷新静默计时且**不判超时**——慢查询（LLM 想很久，如 agent 跑天气脚本）期间
        用户不说话不该触发"静默超时回休眠"，否则答完立刻播"退下"告别语（实测根因）。
        """
        if self.state == SLEEP:
            return "none"
        ...
        if rms2 > speech_pow or engaged:
            self.last_activity = now                   # 房间有语音能量 / 系统应答中 → 刷新计时
        if engaged:
            return "full"                              # 应答中不算"静默"，不判超时
