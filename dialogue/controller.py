...
class DialogueController:
    ...
    def _tools_prompt(self):
        ...
        lines.append("- **说到就要做到**：只要说\"我去查一下/帮你看看/稍等\"这类要查数据的过渡语，"
                     "这一轮**必须**紧跟着输出对应的工具标签（如 <get_gold_history/>）并调用，"
                     "绝不能只说过渡语就结束回答——查询类问题一律要调用工具拿到真实结果后再答"
                     "（2026-09-20 实测：模型只回\"我这就去帮你看看今天的金价\"就停，没调工具）。")
