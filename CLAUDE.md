...
- **LLM 模式工具（可选）** `--tools all|名字列表`（默认关=旧行为零变化，仅 `--brain llm`
  ...
  **"说到就要做到"提示词（2026-09-20 实测）**：工具提示词补一条硬规则——模型只要说
  "我去查一下/帮你看看/稍等"这类查数据的过渡语，这一轮**必须**紧跟着输出对应工具标签并
  调用，绝不能只说过渡语就结束（实测 DeepSeek 只回"我这就去帮你看看今天的金价"就停、没
  调工具——人格"说话要短"与"可调工具"叠加导致"只承诺不行动"）。排查"没调工具"先看该
  轮输出有没有标签；模型可以短，但短在调完工具、答完数据之后。
