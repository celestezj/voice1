...
- **热词纠错在累计 buf 上做，别在 generate 时透传（T16）**：paraformer 流式 generate 返回增量 delta，跨块单词（"神/庙"分两次）片段内匹配不到目标词 → 后端 `_correct()` 统一对 `_partial_buf` 累计文本应用（offline 对整句文本），不在 generate 时传 postprocess_hotword_file。
